...
> - **Payment Link:** identifier in fetch / update / resend / expire calls is `linkId` as a **JSON number**, NOT a string. Resend path is `/link/resendNotification`, NOT `/link/resend`. **Field charsets differ:** `linkName` is alphanumerics ONLY (no spaces — several MIDs reject space as a special character despite the docs); `linkDescription` is alphanumerics + spaces. Both ≥ 3 chars. Sanitize each field with its own regex on the server. Reconcile via `/link/fetchTransaction`, NOT `/v3/order/status`.
