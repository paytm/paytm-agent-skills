...
**Symptom.** Node server returns `ENOTFOUND` (or a custom-mapped error like "Cannot resolve Paytm hostname / DNS error") when calling `/theia/api/v1/initiateTransaction`, `/v3/order/status`, etc. User says "but it was working an hour ago" or "the same code works on my colleague's machine".

**Most likely cause.** Process-state staleness in a long-running Node server. The process is alive, but its internal DNS / network state was poisoned by a transient failure (VPN toggle, laptop sleep/wake, Wi-Fi handover, brief corp-proxy interception). Once Node + libuv + macOS `mDNSResponder` hold onto a negative resolution result, subsequent requests in the same process keep failing — even after the underlying network has fully recovered.

**This is NOT a code bug.** Before changing any code, prove the failure is in code.

### Diagnostic order (do not skip)

1. **Shell DNS:** `dig +short secure.paytmpayments.com` (or `securestage.paytmpayments.com` for staging). Expect an answer. If empty, it's a network / DNS problem — *not* this case; jump to the corp-proxy TLS section below.
...
Only escalate to the code-level workarounds in the **Corp proxy TLS interception** section below once you have confirmed that a freshly-restarted process **also** fails against the same network.
