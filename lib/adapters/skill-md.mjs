...
export function install({ manifest, target, dir, withBackends = false, dryRun = false }) {
  ...
  // 4. Routing manifest (per-target file like CLAUDE.md / SKILL.md / .mdc)
  //    - If routing_file is absolute or `~`-prefixed -> write at that absolute
  //      path (e.g. Cursor's `~/.cursor/rules/paytm.mdc` user-level rule).
  //    - Otherwise -> relative to install_dir.
  if (target.routing_file) {
    const rf = target.routing_file;
    const routingPath = rf.startsWith("~/")
      ? join(process.env.HOME || "", rf.slice(2))
      : rf.startsWith("/")
        ? rf
        : join(dir, rf);
