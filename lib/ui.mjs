...
/**
 * Branded banner. Used by interactive UI AND every subcommand
 * (list-targets / list-skills / etc.) for consistent Paytm framing.
 * `mode` is a short subtitle (e.g. "Agent Skills installer", "list targets").
 */
export function printBanner(version, mode = "Paytm Skill installer") {
  const W = 62;
  const top    = "╔" + "═".repeat(W) + "╗";
  const bottom = "╚" + "═".repeat(W) + "╝";
  const blank  = "║" + " ".repeat(W) + "║";

  const titleInner   = "  " + bold(paytmBlue("Paytm Payments")) + dim("  ·  ") + mode;
  const versionInner = "  " + dim(`bundle: paytm@${version}`);
  // Reassurance line so merchants know this only touches Paytm namespace.
  const scopeInner   = "  " + dim("scope: paytm/ namespace only - does not touch other skills");
  ...
  console.log("  " + paytmNavy("║") + padVisible(scopeInner,   W) + paytmNavy("║"));
  console.log("  " + paytmNavy(blank));
...
export async function printUpgradeNoticeIfAny(currentVersion) {
  try {
    ...
    const cacheDir  = join(homedir(), ".paytm-skills");
    ...
    if (latest && semverGt(latest, currentVersion)) {
      ...
      console.log("  " + dim("Upgrade: npx paytm-skills install --force"));
      console.log("");
    }
